...
function drawBubbleChart(root){
    ...
    //
    var diameter = Math.floor(document.body.offsetWidth / 2);
...
function render(){
  ...
  for (i=0;i<data.length;i++){
    var item = {};
    item.value = data[i];
    item.group = colors[data[i]%colors.length];
...
function load(){
  $.get('/BILLS-115SAHR1625-RCP115-66.txt', (data) => {
    console.log(data.length)
    $('#doc').width(Math.floor(document.body.offsetWidth / 2))
    $('#doc').height(document.body.offsetHeight)

    $('#doc').html(`<pre>${data}</pre>`)
  })
}
